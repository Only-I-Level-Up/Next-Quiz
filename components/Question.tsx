...
const Question = ({ question, widthId, checkEnabled, getAnswer }: Props) => {
  ...
  return (
    ...
    >
      <div className="text-slate-500 text-xl mb-4">
        {widthId ? question.id + ") " : "" + question.text}
      </div>
      ...
      {question.options.map((option) => (
        <label
          ...
          className={`flex justify-start items-center space-x-4 text-gray-500 bg-white shadow-sm rounded-lg my-2 px-4 py-2 border-2 duration-200 border-transparen hover:border-blue-600 cursor-pointer`}
        >
          {isAnswered &&
            (isTrue ? (
